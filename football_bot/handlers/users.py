...
async def open_u(call: CallbackQuery, db: AsyncSession, state: FSMContext):
    if not is_admin(call.from_user.id):
        await call.answer(NO, show_alert=True); return
...
async def list_u(call: CallbackQuery, db: AsyncSession):
    if not is_admin(call.from_user.id):
        await call.answer(NO, show_alert=True); return
    await call.answer()
    users = await us.list_users(db, 30)
    if not users:
        await call.message.edit_text("\u06a9\u0627\u0631\u0628\u0631\u06cc \u0646\u06cc\u0633\u062a.", reply_markup=users_menu_keyboard()); return
    lines = ["\U0001f4cb <b>\u06a9\u0627\u0631\u0628\u0631\u0627\u0646 \u0627\u062e\u06cc\u0631</b>", ""]
    for u in users:
        tag = ("@" + u.username) if u.username else (u.first_name or "-")
        st = BANNED if u.is_banned else OK
        lines.append(f"{st} <code>{u.telegram_id}</code> {tag}")
